...
- (NSString *)yyyyMMddFormatted
{
    ...
    NSString *currentDay = [dateFormatter stringFromDate:self];//当前年月日
    return currentDay;
}
...
+ (NSString*)formattedCreatTimeFromTimeInterval:(long long)time {
    NSDate *createDate = [NSDate dateWithTimeIntervalInMilliSecondSince1970:time];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
...
+ (TimeBucketType)getTheTimeBucket {
    ...
    if ([currentDate compare:[NSDate getCustomDateWithHour:0]] == NSOrderedDescending && [currentDate compare:[NSDate getCustomDateWithHour:6]] == NSOrderedAscending) {
        return TimeBucketTypeEarlyMorning; //凌晨
    } else if ([currentDate compare:[NSDate getCustomDateWithHour:6]] == NSOrderedDescending && [currentDate compare:[NSDate getCustomDateWithHour:12]] == NSOrderedAscending) {
        return TimeBucketTypeMorning; //上午
    } else if ([currentDate compare:[NSDate getCustomDateWithHour:12]] == NSOrderedDescending && [currentDate compare:[NSDate getCustomDateWithHour:14]] == NSOrderedAscending) {
        return TimeBucketTypeNoon; //中午
    } else if ([currentDate compare:[NSDate getCustomDateWithHour:14]] == NSOrderedDescending && [currentDate compare:[NSDate getCustomDateWithHour:18]] == NSOrderedAscending) {
        return TimeBucketTypeAfternoon; //下午
    } else if ([currentDate compare:[NSDate getCustomDateWithHour:18]] == NSOrderedDescending && [currentDate compare:[NSDate getCustomDateWithHour:21]] == NSOrderedAscending) {
        return TimeBucketTypeNight; //晚上
    } else {
        return TimeBucketTypeLateNight; //深夜
    }
}
